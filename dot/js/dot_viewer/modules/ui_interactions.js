/**
 * ui_interactions.js - UI Interaction Module
 * 
 * This module is responsible for user interface operations and interactions.
 * Main responsibilities:
 * - Toggle panel display/hide
 * - Toggle fullscreen mode
 * - Code clear, copy, and paste functionality
 * - UI component state management
 * - SVG icon definition and management
 */
...
export function setupSettingsPanelToggle(buttonElement, settingsPanelContentElement) {
    if (!buttonElement || !settingsPanelContentElement) {
        console.warn("Button element or settings panel content element not found.");
        return;
    }
    buttonElement.addEventListener('click', function() {
        const isHidden = settingsPanelContentElement.classList.contains('hidden');
        if (isHidden) {
            settingsPanelContentElement.classList.remove('hidden');
            this.innerHTML = SVG_ICON_CHEVRON_UP;
            this.title = 'Hide settings';
        } else {
            settingsPanelContentElement.classList.add('hidden');
            this.innerHTML = SVG_ICON_CHEVRON_DOWN;
            this.title = 'Show settings';
        }
    });
    const isInitiallyHidden = settingsPanelContentElement.classList.contains('hidden');
    buttonElement.innerHTML = isInitiallyHidden ? SVG_ICON_CHEVRON_DOWN : SVG_ICON_CHEVRON_UP;
    buttonElement.title = isInitiallyHidden ? 'Show settings' : 'Hide settings';
}
...
export function setupFullscreenToggle(buttonElement, bodyElement) {
    ...
    function updateFullscreenButtonState(isFullscreenActive) {
        ...
        buttonElement.title = isFullscreenActive ? 'Return to normal view' : 'Fullscreen view';
    }
...
export function setupPasteCodeButton(pasteButton, codeInputElement, afterPasteCallback) {
    ...
    pasteButton.addEventListener('click', async () => {
        if (!navigator.clipboard || !navigator.clipboard.readText) {
            alert('This browser does not support clipboard reading.');
...
// CDN panel toggle function removed as we're now using static script tags
